# pylint: disable=too-many-instance-attributes, no-member, invalid-name
...
class AuthenticateTest(TestCase):
    ...
    def test_creates_new_user_if_necessary_for_valid_assertuion(
            self, mock_post
    ):
        mock_post.return_value.json.return_value = {
            'status': 'okay',
...
class GetUserTest(TestCase):
    ...
    def test_returns_none_if_no_user_with_that_email(self):
        backend = PersonaAuthenticationBackend()
        self.assertIsNone(backend.get_user(email='a@b.com'))
